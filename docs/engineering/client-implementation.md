...
### §14.16 — Merge of increment 2 + increment 3 verification (2026-09-04, human-authorized)

**Human authorization (2026-09-04):** "APPROVED — merge Phase 5 increment 2 and begin increment 3 verification." Recorded in Notion as **DEC-54**. This authorizes the increment-2 merge and increment-3 verification work only — not Phase 5 acceptance.

**Merge.** Verified PR #2 head was exactly `69dbba0eaab8dc95e221daee0ec4adcb293193d4` (no intervening changes) with `db-verify` / `full-app-typecheck` / `logic-tests` all `SUCCESS` and `mergeStateStatus: CLEAN`. Squash-merged through the normal protected workflow (`gh pr merge --squash`, no admin bypass) → merge commit `fd0af9483d95f003702519ebd03555e96ee21739` on `main`. Branched `phase-5/increment-3` from updated `main`.

**1. Retained-account discovery/removal — finished the authorized CE-R5 v2 scope.** The DEC-53 pass tracked only one retained account in memory, lost on restart, and overwritten by a second retain. Fixed:
- `client/src/data/local/retained-db-files.ts` (pure, unit-tested) + `driver.native.ts:listRetainedUserIds` (new `expo-file-system` `Paths`/`Directory` API, SDK 54) — lists the SQLite directory and parses `fitney-<userId>.db` filenames, excluding the active user and WAL/SHM/journal sidecars.
- `container.ts:discoverRetainedAccounts` native seam; `context.tsx` now holds `retainedAccounts: string[]` (was a single `string | null`), re-derived from disk once at cold boot (`mergeDiscovered`) and kept correct incrementally by `retire()`/`activate()`/`removeAccountFromDevice()` via pure reducers in `runtime/retained-accounts.ts` (`withRetained`/`withoutRetained`/`visibleRetained`/`assertRemovable` — extracted for unit-testability the same way `SignOutController` was).
- `app/settings.tsx` — the "Retained on this device" card now lists every retained account with its own independent remove-confirm step.
- Adds `expo-file-system` (~19.0.24, `expo install`-locked).

Tests (`retained-db-files.test.ts` x6, `retained-accounts.test.ts` x8, `sign-out-v2.test.ts` +2): multi-account-switch accumulation (never overwrites), app-restart re-derivation via `mergeDiscovered`, active-account exclusion, `assertRemovable`'s refusal of the active account, an unreadable/corrupt retained file (open fails — the exact failure `outstandingForUser` catches into `null`) and that removal never depends on a successful open first.

**2. Hosted GoTrue auth/recovery + WORK-013 + WORK-020 hosted cross-run.** `client/scripts/hosted-verify.mjs` (new, opt-in/manual, no secret — client-safe URL + anon key only) run against real `fitney-dev`, calling the exact `sync_apply` RPC shape `data/remote/gateway.ts` uses. Full results: `docs/engineering/evidence/10-hosted-gotrue-auth-probe.json`, `11-work013-work020-hosted-cross-run.json`.

- WORK-020 hosted cross-run: CONFIRMED. Pushed `client/src/test/golden-vectors.ts`'s exact fixture through `sync_apply` and read the trigger-computed `personal_records`/`weekly_aggregates`/`exercise_weekly_rollups` back: max_load 110, e1RM 129.8333 (`epley`/1), rep_pr `{1:110,5:100,8:102.5}`, session_volume 1430, weekly_working_volume 1430, exercise-weekly best_e1rm 129.8333 — byte-for-byte match to `GOLDEN_EXPECT`. Stronger evidence than WORK-022's earlier hosted check (direct SQL) — this run went through the client's own RPC call shape and a real `auth.uid()`.
- WORK-013 conformance, hosted, confirmed: `operation_id` dedupe/exactly-once (replay returns the cached `duplicate` result, same version); optimistic concurrency (stale `base_version` returns `conflict` plus the current row, no overwrite); cross-tenant denial through `sync_apply` itself (user B gets `rejected`); tombstone delete applies and recompute reacts (max_load moves to the next-highest remaining set); unknown-entity hard reject; idempotent resubmission is byte-identical on the derived rows.
- Hosted auth probe — real, empirical findings (not inspection of config, since that endpoint is unreachable — see below):
  - `@example.com` is rejected as an invalid signup domain (built-in GoTrue denylist).
  - Auth-endpoint rate limiting is active and exhausts quickly under light, non-adversarial automated use (about 4 calls) — confirmed reproducibly across two separate process runs 92s apart.
  - Confirmed discrepancy: hosted effective `minimum_password_length` is 6 (from the actual rejection message), but `supabase/config.toml` declares 8 plus `lower_upper_letters_digits` as the intended SEC-C2-hardened policy. `config.toml` is never pushed to hosted auth settings by `supabase db push` / `db reset --linked` (migrations only) — SEC-C2 hosted auth hardening has not been applied to `fitney-dev`. Routed to `security-identity`/`platform-release` alongside the existing SEC-C2 tracking.
  - Left explicitly unverified (not claimed either way): password complexity enforcement, email-confirmation-required default, recovery-redirect allow-list enforcement — the rate limit blocked a clean probe before these could be observed.
  - Two synthetic users were created directly via `execute_sql` (setting `email_confirmed_at`) only to unblock the `sync_apply`/recompute cross-run after the HTTP signup path was rate-limited — this is a deliberate bypass of the normal path and is not evidence of GoTrue's default confirmation policy.
  - All synthetic rows (`auth.users`, `auth.identities`, and every `public.*` row they created) were deleted after the run — verified 0 leftover rows; security advisor unchanged (only the pre-existing intentional `deletion_receipts` INFO).
- Management API auth-config endpoint (`GET /v1/projects/{ref}/config/auth`) remains genuinely BLOCKED — the connected Supabase MCP exposes `list_projects`/`get_project`/`execute_sql`/`get_advisors`/etc. but no auth-config tool, and this session holds no personal access token to call the Management API directly (never requested — that would need a human-supplied secret). An empty/INFO-only advisor list is explicitly NOT treated as verification of this. Precise unblock path for the human: Supabase Dashboard → project `fitney-dev` (`oaubwbvoaydveguqjovq`) → Authentication → Providers → Email (password length/complexity, confirm-email toggle), Authentication → Rate Limits, Authentication → URL Configuration (Site URL / Redirect URLs — confirm `fitney://auth/callback` is allow-listed), Authentication → Attack Protection (leaked-password / CAPTCHA). No secret needs to be pasted anywhere in this session — reading those pages is enough.

**3. WORK-007 (device) / WORK-010 (Expo SQLite runtime) — genuinely BLOCKED, hardware/access.** Checked for `adb`, an Android emulator, and `xcrun` (iOS Simulator, not available on Windows regardless) in this environment: none present. `app.json` intentionally restricts `"platforms": ["ios","android"]` (no `web`) per the product boundaries (CLAUDE.md: web/desktop authoring out of scope) and `expo-sqlite`/native modules have no meaningful browser equivalent — adding web support to work around the hardware gap would itself be an unauthorized scope/architecture change, so it was not done. What is available without a device was re-verified in this pass (already-existing logic-tier coverage, not new): offline logging (`features/logging/__tests__/offline-logging.test.ts`), force-close/relaunch recovery modeled as a fresh DB handle over the same persisted file (`account-isolation.test.ts`'s "offline relaunch" and "dispose is idempotent" cases). These prove the persistence/isolation logic is correct; they are not a substitute for real Expo-runtime WAL/prepared-statement guarantees (WORK-010) or real accessibility-tree/Dynamic-Type/contrast/VoiceOver-TalkBack rendering (WORK-007), which need an actual device or simulator. Unblock path: from `client/`, `npx expo start` and scan the QR with Expo Go on a physical iOS or Android phone, or provide access to a macOS host (iOS Simulator) or a machine with Android Studio's emulator.

**Verification (from `client/`):** full-app `tsc` PASS, logic `tsc` PASS, `depcruise src app` 0 err (1 pre-existing warn), `eslint` clean on all touched files, `expo-doctor` 18/18, `jest` 19 suites / 135 tests. Hosted: `sync_apply`/recompute conformance and golden-vector cross-run CONFIRMED against real `fitney-dev` (evidence 10-11). Still device/hosted-unverified: `context.tsx` to `settings.tsx` UI wiring/rendering (WORK-007, no `jest-expo`), real GoTrue confirmation-default/complexity/redirect-allow-list behavior (rate-limited before observable), `expo-sqlite` native runtime guarantees (WORK-010).

#### State (§14.16)

Merge commit `fd0af9483d95f003702519ebd03555e96ee21739` on `main`. Phase 5 **IN PROGRESS** · Foundation exit gate **OPEN** · phases 9/10/11 **LOCKED**. Remaining Foundation-gate blockers: WORK-007, WORK-010 (both hardware/access-blocked in this environment), hosted-auth SEC-C2 gap (routed), password-complexity/confirmation-default/redirect-allow-list (unverified, not claimed). Issue IDs preserved: SEC-OQ-1, SEC-RESID-1, OQ-3, OQ-9/DEP-4, UX-OQ-4, ISS-28, CE-R1, CE-R2.

### §14.17 — Increment-3 continuation under DEC-54: WORK-020 substantiation + remaining WORK-013 hosted scenarios (2026-09-04)

**Human instruction:** continue increment 3 under the existing DEC-54 authorization (no renewed approval needed); (1) substantiate WORK-020 with a direct client-vs-hosted comparison, not two separate comparisons to the same static constant; (2) continue the remaining WORK-013 hosted scenarios using the production `SyncEngine`/gateway wherever feasible, with a specific blocker per unexecuted case.

**1. WORK-020 substantiated.** §14.16's evidence compared the client TS domain code and the hosted server SEPARATELY against the same static `GOLDEN_EXPECT` constants (`recompute.golden.test.ts` vs `hosted-verify.mjs`'s inline copy of those numbers) — never directly against each other in one harness. `client/src/domain/__tests__/recompute.hosted-cross-run.test.ts` closes that gap: it imports and executes the PRODUCTION `recomputeAll()` from `domain/pr.ts` on `GOLDEN_FACTS`, loads the hosted read-back captured in `docs/engineering/evidence/11-work013-work020-hosted-cross-run.json`, and asserts the client's LIVE computed values equal the hosted values directly — max_load, est_1rm (+ formula id/version), session_volume, the rep_pr map, the weekly aggregate, and the exercise-weekly rollup. Runs in CI every time (reads a checked-in evidence file, no network). **7/7 assertions pass.**

**2. Remaining WORK-013 hosted scenarios.** New opt-in `client/jest.hosted.config.cjs` + `client/src/data/sync/__tests__/hosted/sync-hosted.hosted.test.ts` drive the PRODUCTION `data/sync/{push,pull}.ts` functions (`runPush`, `runIncrementalPull`) against the REAL `createSupabaseGateway` on `fitney-dev` — never `FakeGateway`, and never re-implemented as raw RPC calls the way `hosted-verify.mjs` was. `tsconfig.hosted-test.json` (unlike `tsconfig.logic.json`) includes `data/remote/gateway.ts`; `jest.config.cjs` now excludes `__tests__/hosted/` so the default/CI `logic-tests` project never tries to compile it. Full results: `docs/engineering/evidence/12-work013-hosted-scenarios.json`.

- **Clock skew: PASS, deterministic.** A device with its local clock set 10 hours in the past still loses to the device that actually reached the server first — the server `version`, never a client timestamp, decides the winner (confirms ADR-0003's AR-C2).
- **Kill-mid-push replay: PASS, deterministic.** Simulated the exact DB mutation `runPush` performs immediately before its network call (`UPDATE sync_outbox SET state='dispatched'`) WITHOUT ever calling the gateway — modeling a crash between marking dispatched and actually sending. Retry against the real server performs the first real, exactly-once application.
- **Lost transport response + concurrent successor: PASS, deterministic.** Applied an operation directly against the real server (simulating "the request landed, the ack didn't"), left the local outbox thinking it's still `dispatched`, created a genuine concurrent local edit (a pending successor) while it appeared in-flight. Retry against the real server correctly returns `duplicate`; the successor is re-based to the true server version and NOT sent in the same pass.
- **Same-timestamp page boundary: PASS, deterministic.** A single multi-row `INSERT` gives two rows the IDENTICAL Postgres transaction-time `updated_at`. Pre-positioned the local cursor exactly at that shared timestamp (simulating the first row having been the last one applied on a prior page — production's fixed `PAGE=200` isn't reachable without ~200 real filler rows) and confirmed the composite `(updated_at,id)` tie-break correctly fetches the second row without re-fetching the first or skipping it.
- **Concurrent writers: NON-DETERMINISTIC — a real defect found.** 6 of 7 runs behaved correctly (exactly one write applied, the other got `conflict`, final version 2). **1 of 7 runs: BOTH concurrent pushes reported `applied`, and the row's version reached 3** — the second writer's edit silently overwrote the first's, with neither side ever seeing a conflict. Root-caused (SQL read, not lock-traced) to `sync_apply`'s UPDATE branch: `v_current` is read via a plain (non-locking) `SELECT`, and the later `UPDATE ... WHERE t.id = $2` never re-checks `version` in its own WHERE clause — a classic check-then-act (TOCTOU) race under true concurrency. **Reported as [ISS-29](https://app.notion.com/p/3d1e607043bc819db47fc3070a84cd5b) (Notion, High priority)** with the full reproduction and a recommended fix (make the version check part of the UPDATE's WHERE clause itself, or add `FOR UPDATE` to the initial SELECT). **NOT fixed** — `sync_apply` is `security-identity`/`backend-data-engineering`'s owned artifact (`supabase/migrations/20260902090006_security_hardening.sql`); this verification pass did not modify it, per the project's phase-ownership model. Routed alongside CE-RISK-2 (updated in `development-roadmap.md`).

**Two WORK-013 cases NOT run hosted, with a specific blocker each (both are tool-access limits, NOT mobile-runtime limits — nothing here needs a device):**
- **Same-timestamp page boundary at the REAL 200-row page size** (as opposed to the cursor-pre-positioning substitute above): the production `PAGE` constant in `data/sync/pull.ts` is a fixed `200`, not injectable, and this project does not have 200 real rows to spare purely to force a natural page split. The substitute test above exercises the identical cursor-continuation SQL clause without needing them; the ONLY gap is proving it survives an *actual* 200-row page fetch, which is a volume/cost decision, not a capability blocker.
- **Late-transaction-commit reconciliation:** genuinely NOT reproducible with the tools available to this session. The scenario requires transaction A to START before the pull cursor's position, remain UNCOMMITTED while a later transaction B commits and the cursor advances past B, then commit AFTER — so A's row is invisible to the incremental cursor forever without full reconciliation. This needs a session/connection held open across multiple, interleaved operations. The `execute_sql` MCP tool provides no persistent connection or session across calls — each call is independently transactional, so a transaction cannot be opened in one call and committed in a later one while other operations happen in between. Compounding this: `supabase/migrations/20260902090001_init_helpers.sql`'s `set_row_metadata()` trigger unconditionally sets `new.updated_at := now()` on every insert/update with NO override path, so even a fabricated "old-timestamp" row cannot be manufactured after the fact — `now()` for any given call is always transaction-start time, which is necessarily >= every prior call's, making it structurally impossible to place a row's `updated_at` behind an already-observed cursor position without a genuine two-transaction overlap. The reconciliation MECHANISM itself (`runFullReconciliation`, `(id,version)` tuples + `fetchByIds`) is unit-tested against `FakeGateway` already (`account-isolation.test.ts` / `pull.test.ts`, per the existing roadmap note); only this SPECIFIC race precondition is unverifiable from this host.

**Verification (from `client/`):** full-app `tsc` PASS, logic `tsc` PASS, `depcruise` 0 err (1 pre-existing warn), `eslint` clean, `jest` (default/CI config) **20 suites / 142 tests** — the hosted-only suites are correctly excluded from that run and were verified separately (opt-in, live network). All synthetic hosted test data (users, sessions) cleaned up after each round — 0 leftover rows, confirmed.

### §14.18 — Hosted-auth table (2026-09-04)

Every row below is either an **empirical, reproducible observation** against real `fitney-dev` (this session's own tooling) or explicitly marked **UNKNOWN** — no row is inferred from an absence of advisory findings. "Applicable requirement" cites `supabase/config.toml`'s declared intent (owned by `security-identity`/`platform-release`, SEC-C2 / TASK-4) where one exists.

| Control | Observed on `fitney-dev` | Applicable approved requirement | Proposed change (NOT applied) | Verification method |
|---|---|---|---|---|
| `minimum_password_length` | **6** — empirical: `signUp` with a 3-char password returned "Password should be at least 6 characters." | `config.toml`: `8` (SEC-C2) | Dashboard: Authentication → Providers → Email → Minimum password length = `8` (or `supabase config push` once linked with write access) | Re-run the empirical probe (a 7-char password) after the change; expect rejection |
| `password_requirements` (complexity) | **UNKNOWN** — the one syntactically-valid-length probe that could have tested this (`weakpw1`, no uppercase) was rate-limited before a content-validation result came back | `config.toml`: `lower_upper_letters_digits` (SEC-C2) | Dashboard: Authentication → Providers → Email → Password Requirements = "Lowercase, uppercase letters and digits" | Empirical probe with a compliant-length, low-complexity password once outside the rate-limit window; or a direct dashboard read |
| `enable_confirmations` (email confirmation required) | **UNKNOWN** — no real signUp reached a state where this was observable (rate-limited); the two synthetic accounts used for the WORK-013/020 cross-run were confirmed via a direct `execute_sql` bypass, which is a deliberate workaround and **not** evidence of the default | `config.toml`: `false` explicitly marked LOCAL ONLY; hosted "MUST be true" per SEC-C2/SEC-REQ-AUTH-* — but that requirement is scoped to the **non-dev** project under **TASK-4** (see reconciliation below), not necessarily binding for a dev project | Dashboard: Authentication → Providers → Email → Confirm email = ON, if dev/prod parity for testing is wanted | A clean signUp probe outside the rate-limit window, checking whether `data.session` is null and a confirmation is required (device runbook §0 does this manually) |
| Auth rate limits (`sign_in_sign_ups`, `email_sent`, etc.) | **CONFIRMED ACTIVE and tight** — exhausted within ~4-8 signUp/recovery calls in under a minute; still blocked 92s later in a separate process | `config.toml` declares specific intended values (`email_sent=10`/hr, `sign_in_sign_ups=30`/hr, `token_refresh=150`/hr, `token_verifications=30`/hr) | If these specific values matter for testing throughput, push them; the observed tightness is not obviously consistent with a 10/hr email budget exhausting in under a minute, suggesting `fitney-dev` may still be on Supabase's platform DEFAULT limits, not the declared ones | Dashboard read (Authentication → Rate Limits); a carefully paced (~1/min) empirical probe once clear of the current block would map the actual window |
| `additional_redirect_urls` (recovery/confirm deep-link allow-list) | **UNKNOWN on `fitney-dev` specifically** (Management API blocked). Static `config.toml` read (round 1) shows only `http://localhost:19006` declared — the app's real deep link `fitney://auth/callback` is absent from the DECLARED list. A live `resetPasswordForEmail` probe with that redirect returned no synchronous error either way — **inconclusive**, since GoTrue may validate the redirect only when actually generating the email link, not at the API-call level | `config.toml`'s `additional_redirect_urls` should include `fitney://auth/callback` (SEC-C2 / round-1 finding) | Dashboard: Authentication → URL Configuration → Redirect URLs → add `fitney://auth/callback` | Dashboard read; or an end-to-end recovery-link tap on a real device (device runbook §0.5) |
| Leaked-password protection (HaveIBeenPwned) | **UNKNOWN** — not observable via any tool available to this session; not itself expressible in `config.toml` | Referenced by `config.toml`'s SEC-C2 comment as intended, but this is a dashboard-only toggle | N/A pending a dashboard read | Dashboard: Authentication → Providers → Email → "Prevent use of leaked passwords" |
| User-enumeration protection | **Inconclusive, weak positive signal** — the duplicate-signUp probe was rate-limited before returning a distinguishing result; two `resetPasswordForEmail` calls (an existing vs. a non-existent email) returned identically (`error: null` both) — consistent with, not proof of, correct no-enumeration behavior | No explicit `config.toml` setting (architectural GoTrue default behavior) | None proposed | Re-run the duplicate-signUp probe outside the rate-limit window for a stronger signal |
| CAPTCHA / Attack Protection | **UNKNOWN** | Not referenced in `config.toml` at all | N/A pending a dashboard read | Dashboard: Authentication → Attack Protection |
| `site_url` | **UNKNOWN on `fitney-dev`** | `config.toml` declares `http://localhost:19006` — a LOCAL dev value, almost certainly not what a mobile app's project should have set | Set once the app's actual expected origin/scheme is finalized (post-device-testing) | Dashboard: Authentication → URL Configuration → Site URL |
| MFA | Not configured | Explicitly deferred in `config.toml`'s comment ("Deferred for MVP") | None | N/A |

**Reconciling `fitney-dev` findings with TASK-4's non-dev scope.** [TASK-4](https://app.notion.com/p/Hosted-auth-hardening-secret-provisioning-Edge-Function-dependency-pin-3cfe607043bc81bcb025ed5345eddd71) ("Hosted auth hardening + secret provisioning + Edge Function dependency pin") is explicitly scoped to **the non-dev (production) project** — "Hard gate for any non-dev environment," Executor: Human, currently `Backlog`. `fitney-dev` is, by name and by DEC-50's "dev-only gate" framing, the dev project — **none of the gaps in the table above are a TASK-4 violation**; TASK-4 has not started and does not govern `fitney-dev`. What this table DOES change for TASK-4: it is now empirically confirmed (not merely assumed) that having `config.toml` declare the correct intended values is **not sufficient** — at least one such value (`minimum_password_length`) demonstrably was never applied to a linked hosted project. **Recommendation for TASK-4's own Definition of Done:** when it executes on the non-dev project, add an empirical verification step identical to this table's method (a signUp probe confirming the rejected-password-length boundary, a signUp probe confirming `enable_confirmations`, a redirect-allow-list check) rather than treating "config.toml looks right" or "the dashboard fields were clicked" as sufficient on their own — this session's experience on `fitney-dev` shows that gap goes undetected otherwise.

### §14.19 — Physical-device runbook (WORK-007 / WORK-010)

Prepared for a human executor: [`docs/engineering/device-runbook-work007-work010.md`](device-runbook-work007-work010.md). Covers setup (Expo Go via QR, `.env` population), a real authentication round trip, offline logging, force-close/relaunch recovery, all four sign-out choices (back up / keep / discard / cancel) plus the offline-backup-failure-restores-writes case from the DEC-53 correction pass on a real device for the first time, and retained-account discovery/multiple-accounts/removal/active-account-protection/re-authentication. **The full hosted authentication/recovery flow stays explicitly open** — the runbook notes exactly which specific outcomes (confirmation-required, redirect deep-link success) remain unverified until a human runs it, matching §14.18's UNKNOWN rows precisely rather than assuming an outcome.

### §14.20 — ISS-29 repair: `sync_apply` atomic version check (DEC-55, 2026-09-04)

**Human authorization:** DEC-55 (Notion Decisions, Approved/Human, 2026-09-04) — bounded ISS-29 repair + `fitney-dev` verification, routed through `backend-data-engineering` + `security-identity`.

**1. Fix.** New forward migration `supabase/migrations/20260904200000_sync_apply_atomic_concurrency_fix.sql` (does not rewrite any previously applied migration). Makes the version comparison and the write one atomic compare-and-swap (`UPDATE ... WHERE t.id = $2 AND t.version = $3`) in both the upsert-UPDATE and the tombstone-DELETE branches, instead of a separate non-locking `SELECT` followed by a blind by-`id` `UPDATE`. When the atomic update affects zero rows, `processed_operations` is re-checked FIRST — a concurrent replay of the same `operation_id` correctly reports `duplicate`, never a spurious `conflict` against itself — before falling back to a genuine `conflict` response. The same recheck was added to the INSERT path's `unique_violation` handler, fixing a **second real gap found while designing the regression**: a concurrent replay racing a fresh insert was previously misreported as `rejected` instead of `duplicate`. Preserved exactly: `security invoker` (RLS), the ownership-column resolution, the entity/op whitelist, the F-2 structured-reject exception handling, the `processed_operations` idempotency shape, and the `{applied|duplicate|conflict|rejected}` response contract. `trg_row_metadata` untouched.

**2 & 3. Regression, gated in CI.** `client/src/data/sync/__tests__/hosted/sync-apply-concurrency.hosted.test.ts` (targets the local Supabase stack by default — CI needs no secret). Full red/green record: `docs/engineering/evidence/13-iss29-fix-regression.md`.
- **Red run** (temporarily reverted to the pre-fix function locally): two concurrent UPDATEs on the same base_version failed 4 of 5 runs (both `applied`, version reached 3); update/tombstone contention, and both operation_id-replay cases, failed 3 of 3 runs each. Confirms all four were real, reproducible gaps.
- **Green run** (fix restored): 3 consecutive clean full runs, 5/5 passing every time — (a) two concurrent UPDATEs → exactly one `applied`, one `conflict`, version 1→2 never →3; (b) **a losing COMPLETED-session edit verified recoverable through the production client** (`runPush`): parked, the loser's payload preserved verbatim in `sync_conflicts`, the local row reflects the winner, outbox clean; (c) update/tombstone contention → exactly one winner; (d)+(e) concurrent replay of the same `operation_id` on both the UPDATE and fresh-INSERT paths → exactly one `applied`, one `duplicate`, never a raw exception or a misreported rejection.
- `supabase test db` still **68/68** (no regression to the existing pgTAP suite); `supabase db lint` clean.
- **CI-gated**: `db-verify.yml` now runs this regression (plus the two WORK-013 closures below) against the SAME local Postgres it already provisions, via a new `actions/setup-node` + `client/` `npm ci` + `jest --config jest.hosted.config.cjs` step sequence — `db-verify` (a required check) now fails if this fix regresses. Per DEC-55: "random repeated runs alone are insufficient," so the gate asserts the exact invariant (one applied / one conflict / one version increment), not just pass/fail noise.

**5. Two more WORK-013 gaps closed in the same pass.**
- **Real 200-row page boundary** (`page-boundary-200.hosted.test.ts`): a bounded, cheap synthetic fixture — 201 rows for one user in a single bulk `INSERT` (one Postgres transaction ⇒ one shared transaction-time `updated_at` for all of them), spanning the ACTUAL `PAGE=200` boundary (not the cursor-pre-positioning substitute from §14.17). All 201 applied exactly once, cursor correct. PASS.
- **Late-transaction-commit reconciliation** (`late-commit-reconciliation.hosted.test.ts`): a session-capable local Postgres harness — two real `pg` connections (added `pg`/`@types/pg` as client devDependencies), one held open across an explicit `BEGIN`/`COMMIT` while the other commits a chronologically-later row — reproduces the scenario for real. Confirmed: incremental pull never recovers the late-committing row (its `updated_at` is behind the cursor by the time it lands); `runFullReconciliation` (id/version-based) does. **Hosted-access limitation preserved as explicitly requested**: this two-long-lived-connection technique is still not available against `fitney-dev` through any tool this session has (no direct Postgres connection string, no persistent `execute_sql` session) — the mechanism is proven correct via the local stack this project already provisions for CI, not hosted, and that distinction is retained rather than glossed over.

**4. Applied to `fitney-dev` (dev only, after local verification passed).** `apply_migration` (Supabase MCP) — the exact migration body, no `db reset`, no unrelated auth-setting change. Security advisor unchanged (only the pre-existing intentional `deletion_receipts` INFO) before and after. Re-ran the hosted concurrency regression against `fitney-dev` itself: **4 additional clean runs, 5/5 (or the isolated race case alone) every time** — the fix holds under real hosted network latency, not just locally. Synthetic test data cleaned up afterward — 0 leftover rows, confirmed.

**6. WORK-020 hosted evidence refreshed.** Re-ran `client/scripts/hosted-verify.mjs` against `fitney-dev` post-fix: `matchesClientGoldenVectors: true`, `idempotent: true` — all values unchanged (the fix only changes concurrency-race behavior, never the computed recompute values). `docs/engineering/evidence/11-*` annotated with the refresh confirmation.

**Verification (from `client/`):** full-app `tsc` PASS, logic `tsc` PASS, `depcruise` 0 err (1 pre-existing warn), `eslint` clean, `jest` (default/CI config) unaffected (still 20 suites / 142 tests — the new hosted-tier files are correctly excluded from that project and were run separately, locally and against `fitney-dev`).

**ISS-29 status: RESOLVED** (Notion) — root cause, fix, before/after evidence, and hosted confirmation all recorded. **Phase 5 remains IN PROGRESS; Foundation exit gate remains OPEN; phases 9/10/11 remain LOCKED; PR #3 not merged.**
